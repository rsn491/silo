...
A CLI tool for managing isolated Git workspaces for multi-agent development. Silo lets multiple AI agents (Claude Code, OpenCode, Codex, Droid) work simultaneously on the same repository without interfering with each other, by creating separate Git worktrees or clones for each agent.
